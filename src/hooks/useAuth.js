...
export const useAuth = () => {
  ...
  const login = async ({ username, password }) => {
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/public/auth/signin`,
        { username, password },
      );
  ...
  const register = async ({ username, email, password }) => {
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/public/auth/signup`,
      ...
      if (err?.response?.data?.errors) errors = err.response.data.errors;
        ...
        errors = [
          {
            msg: "Ocurrió un error inesperado, intentalo de nuevo en unos minutos",
            path: "register",
          },
        ];
  ...
  const validateEmail = async (token) => {
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/user/verify-email/${token}`,
      );
      console.log(response);
  ...
  const resendEmail = async (email) => {
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/user/resend-email/${email}`,
      );
      console.log(response);
      return { resend: true };
    } catch (err) {
